...
function login() {
    ...
    if (isFirstLogin) {
        ...
        if (users[username] && password === users[username]) {
            ...
            showWelcomeScreen();
            loadMembers(); // تحميل الأعضاء عند تسجيل الدخول
        } else {
...
function showWelcomeScreen() {
    ...
    setTimeout(() => {
        ...
        renderMembers(); // تحديث عرض الأعضاء عند الدخول إلى التطبيق
    }, 3000); // 3 ثواني
}

function loadMembers() {
    // تحميل بيانات الأعضاء الخاصة بالمستخدم الحالي
    if (members[currentUser]) {
        members = members[currentUser];
    } else {
        members = {}; // إذا لم يكن هناك أعضاء، نعيد تعيينها لكائن فارغ
    }
}
...
function saveChanges() {
    renderMembers(); // إعادة حساب الإجمالي عند حفظ التغييرات
